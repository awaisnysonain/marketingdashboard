...
export default function NoblToplinePage() {
  ...
  const { dates, summaryByDate, channelNames, channelByDateCh, regionNames, geoByDateRg, subsByDate, kpi } = useMemo(() => {
    ...
    return { dates: allDates, summaryByDate, channelNames: [...chSet].sort(), channelByDateCh, regionNames: [...rgSet].sort(), geoByDateRg, subsByDate, kpi: { totalRev, totalSpend, mer, totalOrders, totalNew, totalSubRev } };
  }, [data]);
      ...
      {loading ? <Skeleton /> : error ? <ErrorBox msg={error} onRetry={load} /> : (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(170px, 1fr))', gap: 12, marginBottom: 20 }}>
            ...
            <KpiCard label="Sub Revenue" value={fmt$(kpi.totalSubRev)} fullValue={fmtFull$(kpi.totalSubRev)} />
          </div>
