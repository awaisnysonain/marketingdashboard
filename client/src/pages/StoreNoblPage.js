...
export default function StoreNoblPage({ showToast }) {
  ...
  return (
    ...
    <div className="page-stack">
      ...
      <PageIntro
        actions={<span className="badge badge--accent">🇺🇸 US + 🇪🇺 EU — Combined</span>}
      />

      {loading ? <Skeleton /> : error ? <ErrorMsg msg={error} onRetry={load} /> : (
        <>
          <div className="card card--pad" style={{ display:'flex', alignItems:'center', gap:8, fontSize:12 }}>
            <span className="badge badge--accent">ℹ EU included</span>
          ...
          <div className="section">
            <div className="section__title">PERIOD TOTALS</div>
            <div className="page-kpi-grid">
              <KpiCard label="Order Revenue" value={fmt$(totals.revenue)} fullValue={fmt$(totals.revenue)} tooltip={TIP.orderRevenue} color="nobl" commentTarget={{ targetType: 'kpi', targetKey: commentTargetKey('order_revenue'), targetLabel: 'Order Revenue' }} />
              ...
              <KpiCard label={L.activeSubs} value={fmtNum(subStats.active||0)} fullValue={String(subStats.active||0)} tooltip={TIP.activeSubs} color="nobl" commentTarget={{ targetType: 'kpi', targetKey: commentTargetKey('active_subs'), targetLabel: L.activeSubs }} />
            </div>
          </div>
...
function ChannelsTab({ channels, chAgg }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      ...
      <div className="section">
        <div className="section__title">REVENUE BY CHANNEL</div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))', gap:12 }}>
          {chAgg.map(ch => (
            <KpiCard
              ...
              accent={CHANNEL_COL[ch.channel] || NOBL_ACCENT}
            />
          ))}
        </div>
      </div>
      ...
      <ChartPanel title="Daily Channel Data — Every Day" subtitle="Each row = one day × one channel. Sort and filter to explore.">
        <PaginatedSheetTable
...
function RegionsTab({ geo, geoAgg }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      {/* Region cards */}
      <div className="section">
        <div className="section__title">REVENUE BY REGION</div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))', gap:12 }}>
          {sortedGeo.map((r, i) => (
            <KpiCard
              key={r.region}
              label={r.region}
              value={fmt$(r.revenue)}
              sub={`Spend ${fmt$(r.spend)} · ${fmtRatio(r.mer)} MER${totalRev > 0 ? ` · ${Math.round((r.revenue / totalRev) * 100)}% of rev` : ''}${r.region === 'EU' ? ' · ✓ in totals' : ''}`}
              accent={r.region === 'EU' ? NOBL_ACCENT : GEO_COL[i % GEO_COL.length]}
            />
          ))}
        </div>
      </div>
      ...
      {/* Daily geo table */}
      <ChartPanel title="Daily Regional Data — Every Day" subtitle="Each row = one day × one region (EU always shown)">
        <PaginatedSheetTable
...
function EmailTab({ email, emailTotals }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      <div className="section">
        <div className="section__title">EMAIL TOTALS</div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(165px,1fr))', gap:12 }}>
